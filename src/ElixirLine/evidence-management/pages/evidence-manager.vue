...
export default {
  ...
  async mounted() {
    try {
      ...
      await this.loadAllEvidences(); // Carga evidencias de todas las tareas
    } catch (error) {
    ...
    async loadAllEvidences() {
      try {
        ...
        for (const task of this.tasks) {
          try {
            const res = await evidenceService.getEvidencesByTaskId(task.id);
